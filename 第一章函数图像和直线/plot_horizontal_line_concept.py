...
# ============ 上方：概念示意图 ============
# 创建一个大方形区域放上部分
ax_top = fig.add_axes([0.05, 0.52, 0.9, 0.38])
ax_top.set_xlim(0, 10)
ax_top.set_ylim(0, 6)
ax_top.axis('off')
ax_top.set_title('Horizontal Line Test (Level 3)', fontsize=18, fontweight='bold', pad=15)

# 左侧示意图 - 固定 y
ax_left = fig.add_axes([0.08, 0.55, 0.25, 0.30])
...
ax_left.set_title('Fix y (固定 y)', fontsize=14, fontweight='bold')
...
# 中间箭头
ax_mid = fig.add_axes([0.36, 0.60, 0.08, 0.20])
ax_mid.axis('off')
ax_mid.annotate('', xy=(0.9, 0.5), xytext=(0.1, 0.5),
                arrowprops=dict(arrowstyle='->', lw=3, color='darkblue'),
                xycoords='axes fraction')
ax_mid.text(0.5, 0.8, 'Step 1\nStep 2\nStep 3', fontsize=10, ha='center',
            va='top', transform=ax_mid.transAxes)

# 中间示意图 - 画水平线
ax_center = fig.add_axes([0.47, 0.55, 0.25, 0.30])
...
ax_center.set_title('Draw y = constant\n(画水平线)', fontsize=14, fontweight='bold')
...
# 右侧箭头
ax_right = fig.add_axes([0.75, 0.60, 0.08, 0.20])
ax_right.axis('off')
...
# 右侧文字
ax_equation = fig.add_axes([0.82, 0.55, 0.15, 0.30])
ax_equation.axis('off')
ax_equation.text(0.5, 0.7, 'Solve:', fontsize=12, ha='center', fontweight='bold',
                 transform=ax_equation.transAxes)
ax_equation.text(0.5, 0.45, r'$f(x) = y$', fontsize=16, ha='center',
                 fontweight='bold', transform=ax_equation.transAxes)
ax_equation.text(0.5, 0.15, 'How many\nsolutions?', fontsize=11, ha='center',
                 transform=ax_equation.transAxes)

# ============ 下方：三情况对比 ============

# Case 1: 一点 (One point) - PASS
...
ax1.set_title('Case 1: One Point (一点)', fontsize=13, fontweight='bold', color='green')
ax1.grid(True, alpha=0.3)
ax1.axhline(y=0, color='black', linewidth=0.5)
ax1.axvline(x=0, color='black', linewidth=0.5)
# Result box
ax1.text(0.5, -0.12, r'$\checkmark$ Yes: 1-to-1', fontsize=12, ha='center',
         transform=ax1.transAxes, color='green', fontweight='bold',
         bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))

# Case 2: 两点 (Two points) - FAIL
...
ax2.set_title('Case 2: Two Points (两点)', fontsize=13, fontweight='bold', color='red')
ax2.grid(True, alpha=0.3)
ax2.axhline(y=0, color='black', linewidth=0.5)
ax2.axvline(x=0, color='black', linewidth=0.5)
# Result box
ax2.text(0.5, -0.12, r'$\times$ No: Not 1-to-1', fontsize=12, ha='center',
         transform=ax2.transAxes, color='red', fontweight='bold',
         bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7))

# Case 3: 零 (No point) - PASS (but y not in range)
...
ax3.set_title('Case 3: No Point (零)', fontsize=13, fontweight='bold', color='blue')
ax3.grid(True, alpha=0.3)
ax3.axhline(y=0, color='black', linewidth=0.5)
ax3.axvline(x=0, color='black', linewidth=0.5)
# Result box
ax3.text(0.5, -0.12, r'$\checkmark$ Yes: 1-to-1', fontsize=12, ha='center',
         transform=ax3.transAxes, color='blue', fontweight='bold',
         bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
ax3.text(0.5, -0.22, '(y not in range / y 不在值域)', fontsize=9, ha='center',
         transform=ax3.transAxes, color='gray')

# ============ 底部总结 ============
fig.text(0.5, 0.01,
         'Summary: Horizontal Line Test determines if f is One-to-One (一对一函数)',
         ha='center', fontsize=14, style='italic', color='darkblue')

plt.savefig('/Users/liutao/Documents/Obsidian/微积分/第一章函数图像和直线/imgs/horizontal_line_concept.png',
